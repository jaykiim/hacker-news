  ...
  _createClass(NewsDetailView, [{
    ...
    value: function render() {
      ...
      api.getData(function (data) {
        ...
        _this2.store.makeRead(Number(id)); // 현재 글 읽음 처리


        _this2.setTemplateData("comments", _this2.makeComment(comments));

        _this2.setTemplateData("currentPage", String(_this2.store.currentPage));

        _this2.setTemplateData("title", title);

        _this2.setTemplateData("content", content);
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "4428" + '/');
